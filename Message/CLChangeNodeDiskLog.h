#ifndef CLCHANGENODEDISKLOG_H
#define CLCHANGENODEDISKLOG_H
...
class CLChangeNodeDiskLog : public CLMessage
{
public:
	CLChangeNodeDiskLog() : IPType(0), IPLength(0), IPAddress(NULL), lengthOfHostname(0), hostname(NULL), capacityBeforeModify(0), capacityAfterModify(0)
	{}
	~CLChangeNodeDiskLog()
	{
		if(NULL != IPAddress)
	...
	void init(int type, int len1, char *address, int len2, char *name, long long c1, long long c2)
	{
		IPType = type;
		IPLength = len1;
		if(NULL != IPAddress)
	...
	bool operator==(const CLChangeNodeDiskLog &Log) const
	{
